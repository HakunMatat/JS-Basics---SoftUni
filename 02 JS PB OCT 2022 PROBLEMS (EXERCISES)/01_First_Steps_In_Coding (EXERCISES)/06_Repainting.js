function Repaiting(input) {
    ...
    let totalSumNaylon = (Number(input[0]) + squareMeters) * protectiveNaylonPerSquareMeter // 18
    ...
    let totalCostOfMaterials = totalSumNaylon + totalSumPaint + totalSumDesolvingPaint + bagCost

    // Total Cost Of Workers

    let amountOfWorkers = Number(input[3])
    let totalCostForWorkers = (totalCostOfMaterials * workerCostInPercentage) * amountOfWorkers
